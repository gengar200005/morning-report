...
#### 1️⃣ [최우선] `claude/session-start-hook-Lv8YN` → main 머지 PR (5-10분)

오늘 #2 세션 5커밋 (`92f20fc` SessionStart hook + `ff064f8` parser/ACTION/CSS
...
#### 2️⃣ UBATP 알림 시스템 E2E 테스트 (30분, 세션 #4 부터 이월)
```bash
...
- **CLAUDE Project 지침 v3.5 → v3.6 갱신** — 오늘 추가된 `is_new` /
  `new_a_entries` 필드 반영 (필요 시)
