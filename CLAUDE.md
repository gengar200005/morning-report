...
> - **ADR-006 Rejected**: streak≤10 walkforward. OOS1 (2020-22) ΔCAGR
>   **-19.27%p**, OOS2 (2023-25) +11.72%p — 방향 불일치 = 재현 알파 아님.
...
- 실전 기댓값 +15-20% 유지
- **백테 진입 규칙 = `check_signal=True` 인 모든 A등급 종목 RS 순 top-5
  (streak 무관, median signal_age=6일 — ADR-005 실증, 2026-04-24 PC)**
  - ⚠️ web #2 의 "🆕 첫날만 = 백테 정합" 해석은 **ADR-005 가 반증**.
    실험 A: baseline median signal_age=6일, fresh(≤1일)=19.5%. 실험 C:
    streak≤N 전부 baseline 하회. Extended 진입 = 정상 동작.
...
Entry Candidates 승격). GH Actions Chrome 환경에서 PDF 검증 후 PR 생성.

검증 체크:
- Section 04 Entry Candidates 녹색 강조 + LIG넥스원 1종목 표시 정상
- Section 05 Trend Watch · Top 5 (재번호 적용) — V위에 🆕 신규 뱃지
- 카드 찢김 0건 (특히 두산에너빌리티 카드)
- ACTION 라인 "돌파 · 거래량 수동 확인" 표시
- 재번호 06/07/08 (Holdings/Macro/Market Context) 정상

#### 3️⃣ [차순위] Entry Candidates 섹션 정당성 재검토 (ADR-005 기반)

PR #16 + web #2 머지 후 필수. 현 Section 04 Entry Candidates 는 "🆕 신규
편입만 = 백테 진입 규칙 일치" 전제. ADR-005 실증:
- baseline median signal_age = **6일** (fresh 1일차는 19.5%)
- signal_days ≤ N 필터 전부 baseline CAGR 하회
- Extended 진입은 알파의 일부 (소실 아님)

→ "🆕 만 매수 후보" 는 리포트 UX 왜곡. 옵션:
- (A) Section 04 재설계 = "오늘 Top 5 (RS 순) · 빈 슬롯만큼 분산" 로 변경
- (B) Section 04 유지 + "🆕 우선순위만 표시, 실제 매수는 Top 5 전체 후보"
  로 설명 문구 정정
- (C) Section 04 폐기 → 05 Trend Watch 로 복귀

별도 ADR (후보 ADR-008) 작성 + 웹 Claude Project 지침 v3.6 영향 검토.

#### 4️⃣ ~~UBATP 알림 시스템~~ — **폐기 (ADR-007, 2026-04-24 PC)**

ADR-007 결정으로 장중 알림 전체 불채택. `claude/session-start-UBATP` 브랜치
삭제 대상. push 권한 필요 → 마스터 명시 승인 시 삭제.

#### 5️⃣ ADR-008 후보 — 박스권 조건부 섹터 게이트 (1-2시간)
...
(원 번호 "ADR-005 후보" → ADR-005 번호 점유로 재번호)
...
- **브랜치 잔여 정리**: `phase3-backtest`, `fix-error-handling-riAYS`,
  `claude/session-start-UBATP` (ADR-007 폐기). 워크플로 수정 포함 브랜치
  4개는 보존 (action flow 규칙)
- **CLAUDE Project 지침 v3.6 의 Entry Candidates 정합성 재검토** — ADR-005
  기반. `is_new` 필드는 유지하되 "first-day only 매수 후보" 문구는 수정
  필요. v3.7 또는 v3.6 amendment.
- ~~**Instructions v3.3 → v3.6 개편** (draft 002)~~ — **Rejected** (PC 세션).
  draft 추가분이 전부 ADR-005/007 "기각 결정" 의 가상 방어 규칙이라
  규칙 비대화. 실제 사건 발생 시에만 추가.
...
- [ADR-005] Entry timing 실증 — fresh-signal 필터 **기각**, baseline 유지 — `docs/decisions/005-entry-timing-diagnosis.md`
  ...
  - 주의: web #2 의 "🆕 A등급 첫날 = 백테 정합" 해석을 본 ADR 이 실증 반증.
- [ADR-006] signal_days ≤ 10 walkforward — **기각** — `docs/decisions/006-streak-le10-residual-investigation.md`
  - 실험 E: OOS1 (2020-22) ΔCAGR **-19.27%p** / OOS2 (2023-25) ΔCAGR **+11.72%p** → 방향 불일치.
  - 수용 기준 C1/C2 미달. H1/H4 (우연 + overfitting) 지지, H3 (MDD 방어) 기각.
