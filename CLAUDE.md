...
### ⏭️ 다음 세션 진입점 (2026-04-23 예정, 웹 Claude Code)

**주 작업**: **장중 실시간 알림 시스템 구축** (2.5~3.5시간)
- 상세 플랜: [docs/plans/001-alert-system-setup.md](docs/plans/001-alert-system-setup.md)
- 사용자 준비물: 텔레그램 봇 토큰 + chat_id (또는 디스코드 webhook)
- 환경: 로컬 PC Task Scheduler 권장 (`docs/environment-guide.md` 참조)

**세션 시작 플로우**:
```
1. git pull (로컬) 또는 GitHub 연결 (웹)
2. /session-start         ← Claude 자동으로 상태 파악
3. "텔레그램 봇 토큰 준비됐어?" 확인
4. 준비됐으면 Step 1(notifier.py) 착수
```
...
- **섹터별 성과 분해** (위험 분산 판단, 2시간)
- **생존편향 정량화** (2015 상폐주 리스트 확보, 5+시간)
- **main 머지** (pyyaml 추가 후, GitHub Actions 테스트 + 머지)
