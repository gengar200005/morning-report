...
# ── Claude API 호출 ───────────────────────────────────
def call_claude(us_data, kr_data, sector_data="", holdings_data=""):
...
═══════════════════════════════════════
[투자 전략 — Minervini Trend Template v1]
═══════════════════════════════════════
- 유니버스: 대형주 FCF 흑자 10종목 이내
- 손절 -7% | 익절: 트레일링 스탑 (고점 대비 -10%)
- 1종목 포트 20~25% | 분할진입 1차 50%

■ Minervini 코어 조건 (①~⑧ 모두 충족 필수)
  ① 현재가 > MA50
  ② 현재가 > MA150
  ③ 현재가 > MA200
  ④ MA50 > MA150
  ⑤ MA150 > MA200
  ⑥ MA200 상승 추세 (현재 MA200 > 1개월 전 MA200)
  ⑦ 52주 저점 대비 +25% 이상
  ⑧ 52주 고점 대비 -25% 이내
...
■ 등급 산정
  코어(①~⑧) + 게이트(⑨⑩) 10개 모두 충족이 전제.
  미충족 시 D등급 — 편입 부적합, 코멘트 불필요.
  A등급: 10개 충족 + 보조 2개 = 12/12 → 진입 검토
  B등급: 10개 충족 + 보조 1개 = 11/12 → 조건부 대기
  C등급: 10개 충족 + 보조 0개 = 10/12 → 진입 가능, 확신도 낮음
...
1단계: 정량 채점 — Minervini 12항목 O/X 판정 + 등급(A/B/C) 확정
...
1. [등급] X등급 (N/12) — 미충족 항목: ⑪RS, ⑫수급 등 구체적 나열
...
- 액션 유형은 4가지로 제한:
  ✅ 진입 검토 (A등급 — 12/12)
  ⏳ 조건부 대기 (B등급 — 11/12) — 미충족 보조 조건 명시
  ⚠️ 확신도 낮음 (C등급 — 10/12) — 보조 조건 2개 모두 미충족
...
kr_data.txt의 개별 종목에는 MA50·MA150·MA200이 포함된다 (Minervini Trend Template).
...
- 이평선(MA50/150/200) 수치는 반드시 실제 데이터 기반. 추정/계산 금지.
</데이터 품질 규칙>
...
보고서 맨 앞에 제목·부제·태그라인을 절대 추가하지 말 것. ## ☀️ ① 이촌동 날씨 섹션부터 바로 시작.
...
## 🔍 ⑤ 스크리닝 결과
{must_warning}
⚠️ 국장 데이터 【Minervini 스크리닝 결과】의 A/B등급 종목 전부 포함. 누락·등급 재평가·추가 금지.
⚠️ C등급은 종목명 나열만 있으므로 스크리닝 섹션에서 제외. ⑥ 핵심 포인트에서 언급만 허용.

A/B/C등급 없을 때: ✋ **오늘 진입 보류** — 조건 미충족

A/B/C등급 있으면 종목별로:
### 🅐/🅑/🅒 종목명 (종목코드)
Minervini 체크리스트 표 (현재가/MA50·MA150·MA200/MA정배열/MA200상승/52주고점대비/52주저점대비/RS%/외국인+기관20일수급/코스피60MA/VIX35이하/PER·PBR·ROE/손절가(-7%)/익절(트레일링스탑 고점-10%))
...
## 💼 ⑦ 보유 종목 현황
보유 종목 데이터가 "보유 종목 없음"이면 이 섹션 전체를 "보유 종목 없음" 한 줄로 종결.
있으면 종목별로:
### 종목명 (종목코드) [등급]
- 매수가·현재가·손익률 1줄
- Minervini 코어 통과 / 정배열 / MA200 상승 / 52주 고점 대비 1줄
- 손절가 / 20일 수급 1줄
- ⇒ 추매 시그널 그대로 인용 (❌ 추매 금지 / ⭐ 추매 조건 일부 충족 등)
⚠️ 추매 시그널 텍스트는 holdings_data.txt에서 산출된 규칙 기반 결과. 재평가·왜곡 금지.
⚠️ 보유 종목은 ⑤ 스크리닝(신규 진입 후보)과 별도 섹션. 혼동 금지.
...
if __name__ == "__main__":
    ...
    try:
        ...
        sector_data = ""
    try:
        holdings_data = read_github_file("holdings_data.txt")
        print("✅ 보유 종목 데이터 로드 완료")
    except Exception as e:
        print(f"⚠️ 보유 종목 데이터 없음 (섹션 ⑦ 생략): {e}")
        holdings_data = ""
    print("✅ 데이터 로드 완료")

    print("🤖 Claude API 리포트 생성 중...")
    report = call_claude(us_data, kr_data, sector_data, holdings_data)
    print("✅ 리포트 생성 완료")
    print("\n" + "=" * 60)
    print(report)
    print("=" * 60 + "\n")
