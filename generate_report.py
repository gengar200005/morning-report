...
- 진입 체크리스트 (6항목):
  ...
  ⑤ VIX 20 이하
  ⑥ 저항→지지 전환 확인
- 등급: A(5~6개)=진입 검토 / B(3~4개)=조건부 대기 / C(0~2개)=편입 부적합
...
<등락 해석 규칙>
- 1일 등락폭 ±2% 이내: "보합" 또는 "소폭 변동". 추세/모멘텀 판단 불가.
- 1일 등락폭 ±2~5%: "변동성 확대"로 기술. 추세 판단은 5일+ 데이터 필요.
...
## 🔍 ⑤ 스크리닝 결과
{must_warning}
⚠️ 국장 데이터 【체크리스트 스크리닝 결과】의 A/B등급 종목 전부 포함. 누락·등급 재평가·추가 금지.
⚠️ B등급도 A등급과 동일하게 반드시 포함.

A/B등급 없을 때: ✋ **오늘 진입 보류** — A/B등급 조건 미충족

A/B등급 있으면 종목별로:
### 🅐/🅑 종목명 (종목코드)
체크리스트 표 (현재가/MA/이평선정배열/거래량/외국인5일/코스피60MA/VIX/섹터ETF/저항지지/PER/PBR/ROE/손절가/목표가)
