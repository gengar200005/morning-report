...
## Project Files (Claude.ai 등록 목록, 4개)
...
**등록 불필요 — 데이터 생성 / CI 단계** (Claude 가 7카드 작성에 직접 인용 X):
- `combine_data.py`, `holdings_report.py`, `kr_report.py`, `gdrive_upload.py`,
  `signals_today.py` — morning_data 생성 / verdict derive / 보유 데이터.
  parser dict 출력만 보면 충분.
- `notion_page_template.json`, `reports/publish_to_notion.py`, `.github/workflows/claude_render.yml`,
  `requirements.txt` — CI 가 자체 실행.
- `CLAUDE.md` — 레포에 있지만 T10/CD60 핵심은 §5 인라인.
