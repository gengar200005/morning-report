...
- Status: **Rejected** (2026-04-24 #2 세션에서 폐기 결정)
- Date: 2026-04-24

## 폐기 사유 (2026-04-24 #2)

본 draft 의 모든 추가분 (진입 게이트 #4, 절대 금지 #5, 자체 체크 #7/#8) 은
**ADR-005 에서 기각된 가설** (fresh-signal 필터, signal_days ≤ N 규칙) 을
Claude 가 재제안하지 못하게 하는 **방어 규칙**.

- v3.3 의 진입 게이트/절대 금지 ★ 항목은 **실제 발생한 drift** (11섹터 파서
  ETF shim, Project Files 누락) 기반이라 박아넣을 가치 있음.
- 본 draft 는 **가상 위험 기반** — 마스터가 Claude 에게 Top5 signal_age 관련
  질문 시 Claude 가 기각된 방향 제안할 수 있다는 *가능성*. 실제 사건 아님.
- 선제적 규칙 비대화는 요점 희석 비용만 발생.

**대안**: ADR-005/007 결정은 `docs/decisions/` + `CLAUDE.md::최근 주요 결정`
+ `SESSION_LOG.md` 에 기록되어 있으므로, 실제 Claude 오답 사건 발생 시에만
해당 사건을 근거로 규칙 한 줄 추가. 선제 반영 불필요.

본 문서는 의사결정 기록 용도로 보존.
