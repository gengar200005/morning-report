...
## 2026-05-19 — 운영 원칙: 시스템 출력 맹신 금지

- **사건**: 현대오토에버 TS 도달 (트레일선 629,100원 하회) 상태였음에도
  모닝리포트 개별 카드 Action에 "추매 금지 · HOLD" 표시 — 매도 신호 누락.
  손절가 534,000원에 전량 청산 후 사후 발견.
- **원칙**: **모닝리포트에서 TS 종료되었음에도 누락됨. 시스템에 대한 의심을 잃지 말 것.**
  자동화 출력이 "정상처럼 보여도" 보유 종목 현재가 vs 트레일선/손절가를
  육안으로 교차 확인하는 습관 유지.
- **후속 조치**: PR #45 (sell_actions NameError) + PR #46 (개별 카드 TS 표시)
  수정 완료. 그러나 코드 수정이 완벽하다는 보장 없음 — 매일 리포트 수령 시
  보유 종목 숫자 직접 확인 루틴 병행.
